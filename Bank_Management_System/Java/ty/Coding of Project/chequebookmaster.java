...
import javax.swing.*;
import java.sql.*;
public class chequebookmaster extends JFrame implements ActionListener
{
  JLabel limg;
 ...
 public chequebookmaster()
 {
  ...
  setTitle("CHEQUE BOOK MASTER FORM");
  setSize(550,500);
  setVisible(true);
 // setBackground(Color.PINK);
   bp=new JLabel("CHEQUEBOOK MASTER",new ImageIcon("Money1.gif"),JLabel.CENTER);
   add(bp);
   setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

  l1=new JLabel("Cheque book no. :");
  ...
  limg=new JLabel(" ",new ImageIcon("plane.gif"),JLabel.LEFT);
 add(limg);
  ...
  try
  {
   ...
   rs=st.executeQuery("select * from chequebook_master");
   rs.next(); 
  }
  catch(Exception e)
  {
 ...
 public void actionPerformed(ActionEvent ae)
 {
  try
  {
   if(ae.getSource()==b1)         //addnew button logic
   {
    t1.setText(" ");
    t2.setText(" ");
    t3.setText(" ");
   ...
   if(ae.getSource()==b2)        //save button logic
   {
    try
    {
     pst=cn.prepareStatement("insert into chequebook_master values(?,?,?,?)");
   ...
   {
    pst=cn.prepareStatement("update chequebook_master set chequebookno=?,customer_name=?,ac_type=? where acno=?");
   ...
   if(ae.getSource()==b6)        //next button logic
   {
    rs.next();
    if(!rs.isAfterLast())
    getData();
   }
   if(ae.getSource()==b5)          //previous button logic
   {
    rs.previous();
    if(!rs.isBeforeFirst());
    getData();
 ...
 {
  chequebookmaster cb=new chequebookmaster();
 }
}